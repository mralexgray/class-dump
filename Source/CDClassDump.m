...
{
    return !!_regularExpression ? !![self.regularExpression firstMatchInString:name
                                                                       options:(NSMatchingOptions)0
                                                                         range:NSMakeRange(0, [name length])] : YES;
}

- (BOOL)containsObjectiveCData { for (CDObjectiveCProcessor *p in self.objcProcessors) if (p.hasObjectiveCData) return YES; return NO; }

- (BOOL) hasEncryptedFiles { for (CDMachOFile *m in self.machOFiles) if (m.isEncrypted) return YES; return NO; }

- (BOOL)hasObjectiveCRuntimeInfo { return self.containsObjectiveCData || self.hasEncryptedFiles; }
...
- (BOOL)loadFile:(CDFile *)file error:(NSError *__autoreleasing *)error {
  ...
  CDMachOFile *machOFile = [file machOFileWithArch:_targetArch]; //NSLog(@"machOFile: %@", machOFile);
  if (!machOFile) {
    if (error) {

      NSString *targetArchName = CDNameForCPUType(_targetArch.cputype, _targetArch.cpusubtype);
      NSString *failureReason = [file isKindOfClass:CDFatFile.class] && [(CDFatFile*)file containsArchitecture:_targetArch]
  ...
  assert([machOFile filename] != nil); [_machOFiles addObject:machOFile]; _machOFilesByName[machOFile.filename] = machOFile;

  if (!self.shouldProcessRecursively) return YES;
  @try {
    for (CDLoadCommand *loadCommand in machOFile.loadCommands) {
      if (![loadCommand isKindOfClass:[CDLCDylib class]]) continue;
      ...
      if (dylibCommand.cmd == LC_LOAD_DYLIB) {
        ...
        path = ![path hasPrefix:loaderPathPrefix] ? path :
                [path stringByReplacingOccurrencesOfString:loaderPathPrefix
                                                withString:machOFile.filename.stringByDeletingLastPathComponent].stringByStandardizingPath;
  ...
  @catch (NSException *exception) {
    ...
    if (error != NULL)
    *error = [NSError errorWithDomain:CDErrorDomain_ClassDump code:0 userInfo:@{
...
{
    [visitor willBeginVisiting]; for (CDObjectiveCProcessor *processor in self.objcProcessors) [processor recursivelyVisit:visitor];
    [visitor didEndVisiting];
}

- (CDMachOFile *)machOFileWithName:(NSString *)name;
{
    NSString *executablePathPrefix = @"@executable_path", *rpathPrefix = @"@rpath", * adjustedName =

    [name hasPrefix:executablePathPrefix] ?
    [name stringByReplacingOccurrencesOfString:executablePathPrefix withString:self.searchPathState.executablePath] :
    [name hasPrefix:rpathPrefix] ? ({  NSString * adjusted;
        ...
        for (NSString *searchPath in [self.searchPathState searchPaths]) {
            ...
            //NSLog(@"trying %@", str);
            if ([NSFileManager.defaultManager fileExistsAtPath:str]) { adjusted = str;
                //NSLog(@"Found it!");
                break;
            }
        }
        adjusted ?: name; //NSLog(@"Did not find it.");

    }) : self.sdkRoot ? [self.sdkRoot stringByAppendingPathComponent:name] : name;
