...
@property (readonly) NSMutableSet *referencedClassNames,
                                  *referencedProtocolNames,
                                  *weaklyReferencedProtocolNames; // Protocols that can be forward-declared instead of imported

@property (readonly) NSArray  *referencedClassNamesSortedByName,
                              *referencedProtocolNamesSortedByName,
                              *weaklyReferencedProtocolNamesSortedByName;

@property (readonly) NSString *referenceString;
@end
...
@implementation CDMultiFileVisitor
{
  ...
  NSMutableSet *_referencedClassNames, *_referencedProtocolNames;
  NSUInteger _referenceLocation;
}

- (id)init { return self          = super.init ?
  _referencedClassNames           = NSMutableSet.new,
  _referencedProtocolNames        = NSMutableSet.new,
  _weaklyReferencedProtocolNames  = NSMutableSet.new, self :nil;
}
...
- (void)didVisitClass:(CDOCClass *)aClass {
  ...
  [super didVisitClass:aClass];   // Generate the regular output

  [self removeReferenceToClassName:aClass.name];   // Then insert the imports and write the file.
  [self removeReferenceToClassName:aClass.superClassName];  NSString *referenceString;
  if ((referenceString = self.referenceString))
  ...
  filename = !self.outputPath ? filename :
             [self.outputPath stringByAppendingPathComponent:filename];
...
- (void)willVisitCategory:(CDOCCategory *)category;
{
  ...
  [super willVisitCategory:category];   // And then generate the regular output
  [self addReferencesToProtocolNamesInArray:category.protocolNames];
}

- (void)didVisitCategory:(CDOCCategory*)category {

  [super didVisitCategory:category];   // Generate the regular output

  [self removeReferenceToClassName:category.className];   // Then insert the imports and write the file.
